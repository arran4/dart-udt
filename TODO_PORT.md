...
- [ ] Define package scope for pure Dart vs optional `dart:ffi` acceleration.
- [ ] Document API parity targets with `udt.h` and unsupported legacy APIs.
- [ ] Decide minimum Dart SDK and supported platforms (Linux/macOS/Windows/Android/iOS/Web where possible).
- [ ] Establish semantic versioning and stability gates before first pub.dev release.

## 2. Source decomposition (from C++ modules)
- [ ] Map these modules into Dart libraries: `api`, `core`, `channel`, `epoll`, `queue`, `buffer`, `packet`, `window`, `cache`, `list`, `ccc`, `md5`, `common`.
- [ ] Replace pointer-heavy data structures with typed Dart classes/records.
- [ ] Define memory/packet layout adapters using `ByteData` and `Uint8List`.
- [ ] Create deterministic serialization/deserialization tests for packet headers and control messages.
...
- [ ] Provide platform support table and known limitations in README.
