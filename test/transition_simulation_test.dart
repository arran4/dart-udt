...
void main() {
  test(
    'simulator tracks cumulative time and adapts ACK/RTO recommendations',
    () {
      const simulator = UdtNetworkTransitionSimulator(
        baseAckIntervalMillis: 10,
      );
  ...
  test(
    'simulator policy-transition matrix covers app/network/power branches',
    () {
      const simulator = UdtNetworkTransitionSimulator(
        baseAckIntervalMillis: 10,
      );
      ...
      expect(snapshots.map((snapshot) => snapshot.timeMillis).toList(), <int>[
        5,
        10,
        15,
        20,
