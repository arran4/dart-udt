...
final class _MatrixFakeTarget
    implements UdtSocketOptionTarget, UdtSocketRuntimeTarget, UdtSocketConnectTarget {
  _MatrixFakeTarget({
...
void main() {
  test('matrix rows are wired into runtime planner/executor expectations', () async {
  ...
  test('dual-stack row falls back to ipv4 while respecting matrix families',
      () async {
    const harness = UdtSocketMatrixIntegrationHarness();
    ...
    expect(row.expectedBindFamilies.containsAll(result.attemptedFamilies), isTrue);
